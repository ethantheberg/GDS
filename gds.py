...
def generate_photonic_crystal(parameters: CrystalParameters):
    ...
    bridge = pg.straight((parameters.bridge_width, parameters.bridge_length))
    defect = pg.straight((parameters.defect_height, parameters.defect_width))
    crystal = pg.straight((parameters.crystal_height, parameters.crystal_width))
    ...
    outline = pg.outline(photonic_crystal, parameters.outline_width, layer=1)
...
def generate_device(parameters: DeviceParameters):
    ...
    for i in [1, 2]:
      ...
      external_electrode = crystal.add_ref(pr.route_quad(crystal.ports[i], external_connector_i.ports[i%2+1], parameters.external_electrode_width, layer=2))
...
def generate_waveguides(
    ...
    center_electrode = waveguides.add_ref(pg.compass_multi((50, electrode_height), {"N": 1, "E": device_count, "W": device_count}))
    ...
    side_electrode = pg.straight((30, electrode_height))
    ...
    bottom_route = waveguides.add_ref(pr.route_sharp(left_electrode.ports[2], right_electrode.ports[2], 30))
...
def generate_pads(
    ...
    left_pad = pads.add_ref(pg.straight((150, 150)))
    center_pad = pads.add_ref(pg.straight((150, 150)))
    right_pad = pads.add_ref(pg.straight((150, 150)))

    pads.distribute(spacing=75)

    left_pad_top = pads.add_ref(pg.compass_multi((150, 120), ports={'E': 2, 'S': 1}))
    left_pad_top.connect('S1', left_pad.ports[1])


    right_pad_top = pads.add_ref(pg.compass_multi((150, 120), ports={'W': 2, 'S': 1}))
    right_pad_top.connect('S1', right_pad.ports[1])

    top_connection = pads.add_ref(pr.route_quad(right_pad_top.ports['W2'], left_pad_top.ports['E2']))
    ...
    waveguides = cell.add_ref(generate_waveguides(device_count, device_spacing, gap_spacing, device_parameters))
    pads = cell.add_ref(pads)

    pads.connect('C', waveguides.ports['C'], -100)

    cell.add_ref(pr.route_quad(pads.ports['C'], waveguides.ports['C']))

    cell.add_ref(pr.route_quad(pads.ports['L'], waveguides.ports['L']))
    cell.add_ref(pr.route_quad(pads.ports['R'], waveguides.ports['R']))
